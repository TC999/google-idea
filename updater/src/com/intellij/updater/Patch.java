...
public class Patch {
  ...
  public boolean validateDeletion(String path) {
    for (String delete : myDeleteFiles) {
      if (path.matches(regexForPath(delete))) {
        return false;
      }
    }
    return true;
  ...
  public static String regexForPath(String path) {
    String regex = "";
    int from = 0;
    int to = 0;
    while ((to = path.indexOf('*', from)) != -1) {
      regex += Pattern.quote(path.substring(from, to));
      regex += ".*";
      from = to + 1;
    }
    return regex + Pattern.quote(path.substring(from));
  }
