...
public class RunManagerImpl extends RunManagerEx implements PersistentStateComponent<Element>, NamedComponent, Disposable {
  ...
  private List<BeforeRunTask> getDefaultBeforeRunTasks(@NotNull RunConfiguration settings) {
    BeforeRunTaskProvider<BeforeRunTask>[] runTaskProviders =
      Extensions.getExtensions(BeforeRunTaskProvider.EXTENSION_POINT_NAME, myProject);
    DefaultBeforeRunTaskChooser[] taskChoosers =
      Extensions.getExtensions(DefaultBeforeRunTaskChooser.EP_NAME, myProject);
    ...
    for (BeforeRunTaskProvider<? extends BeforeRunTask> provider : runTaskProviders) {
      ...
      for (DefaultBeforeRunTaskChooser taskChooser : taskChoosers) {
        taskChooser.changeIsEnabled(task, settings);
      }
      if (task.isEnabled()) {
