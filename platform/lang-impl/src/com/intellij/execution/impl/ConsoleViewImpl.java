...
public class ConsoleViewImpl extends JPanel implements ConsoleView, ObservableConsoleView, DataProvider, OccurenceNavigator {
  ...
  private boolean myShouldStickToEnd;
  ...
  private void flushDeferredText(boolean clear) {
    ...
    final boolean shouldStickToEnd = clear || shouldStickToEnd();
    if (clear) {
  ...
  private boolean shouldStickToEnd() {
    if (myCancelStickToEnd) {
      // By disabling myShouldStickToEnd for one frame, the console view falls behind, so the next
      // time around when we call this method, myShouldStickToEnd will continue to be false.
      myCancelStickToEnd = false;
      myShouldStickToEnd = false;
      return false;
    }

    if (myEditor == null) return myShouldStickToEnd;
    Document document = myEditor.getDocument();
    int caretOffset = myEditor.getCaretModel().getOffset();
    myShouldStickToEnd = document.getLineNumber(caretOffset) >= document.getLineCount() - 1;
    return myShouldStickToEnd;
  }
