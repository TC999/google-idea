...
//      }
      ...
      if (!myActionGroup.isPrimary(action)) {
        ...
        //2. They show up under a popup under a cogwheel button (layoutSecondaries = true, getRightAlignSecondaries = false)
        //    This happens if the layout supports the cogwheel (horizontal, auto layout)
        //3. They show up as regular toolbar buttons (layoutSecondaries = false, getRightAlignSecondaries = false)

        if (getRightAlignSecondaries()) {
          mySecondaryActions.add(action);  //the secondary will be in the main toolbar, but right aligned.
          continue;
        }
        else if (layoutSecondaries) {
