...
/**
 * Template for classes that are NOT children / grandchildren etc.. of: Activities, Fragments, Views, or Services
 * Contains code blocks used for permission guard insertion
 * These classes only support permission checks if there is a variable of type Context in the scope of the sensitive
 */
...
class DefaultTemplate extends BaseTemplate {
    ...
    protected String toCheckOnlyBlock() {
        def block =
        """
        if (android.support.v4.app.ActivityCompat.checkSelfPermission(${developerContext}, ${qualifiedPermissionName})
