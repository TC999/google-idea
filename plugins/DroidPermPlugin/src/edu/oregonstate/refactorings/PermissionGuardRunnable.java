...
/**
 * Runnable for analysis and QuickFix-based transformations of Android source code to use Runtime Permissions that
 * are compatible with Android API 23+.
 *
 * @author Nicholas Nelson <nelsonni@oregonstate.edu> Created on on 6/9/16.
 */
public class PermissionGuardRunnable implements Runnable {
    ...
    private final boolean insertCallback;
    ...
    public PermissionGuardRunnable(Project project, Editor editor, @NotNull String permission, boolean insertRequest) {
        ...
        this.insertCallback = insertRequest;
        updateRefs();
    }
    ...
    public void run() {
        ...
        // generate templates
        final BaseTemplate template = PermissionsUtil.getTemplate(permission, containingClass, selectionModel);

        if (template != null) {
            // add check permission block to target class
            insertPermissionCheck(template);

            // add constant request code to target class
            insertRequestCode(template);

            // add new callback method, or a new branch to an existing callback method, in target class
            if (insertCallback) insertCallback(template);
    ...
    /**
     * Handles inserting the permission check code surrounding the selected block of code.
     *
     * @param template  Template instance as configured in templates.properties
     * @param <T>       Template class derived from {@link edu.oregonstate.templates.BaseTemplate}
     */
    private <T extends BaseTemplate> void insertPermissionCheck(@NotNull T template) {
        List<String> endStrings = new ArrayList<>();
        endStrings = insertEndOfCheck(template.getCheckBlockElse(), endStrings);
        if (selectionModel.hasSelection()) {
            //noinspection ConstantConditions
            endStrings = insertEndOfCheck(selectionModel.getSelectedText(), endStrings);
        }
        endStrings = insertEndOfCheck(template.getCheckBlockAfter(), endStrings);
        document.insertString(selectionModel.getSelectionStart(), template.getCheckBlockBefore());
        //Calculate Start and End offset of modified code
        int startOffset = selectionModel.getSelectionStart() -
                template.getCheckBlockBefore().length();
        ...
        PsiDocumentManager.getInstance(project).commitDocument(document);
        //Format only the code that we've modified
