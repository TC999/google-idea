...
public abstract class MetaManager extends ModelLoader {
  ...
  protected VariationPaletteItem createVariationPaletteItem(Element itemElement, MetaModel model, PaletteItem paletteItem) {
    return new VariationPaletteItem(paletteItem, model, itemElement);
  }

  @NotNull
  protected PaletteGroup createGroup(Element element) {
    return new PaletteGroup(element.getAttributeValue(NAME));
  }
  ...
  protected PaletteGroup loadGroup(Element element) throws Exception {
    PaletteGroup group = createGroup(element);
    ...
    for (Object child : element.getChildren(ITEM)) {
      ...
      if (!itemElement.getChildren().isEmpty()) {
        ...
        if (itemElement.getAttribute("title") != null) {
          paletteItem = createVariationPaletteItem(itemElement, model, paletteItem);
        }
        group.addItem(paletteItem);

        for (Object grandChild : itemElement.getChildren(ITEM)) {
          group.addItem(createVariationPaletteItem((Element)grandChild, model, paletteItem));
        }
      }
      else {
        group.addItem(paletteItem);
